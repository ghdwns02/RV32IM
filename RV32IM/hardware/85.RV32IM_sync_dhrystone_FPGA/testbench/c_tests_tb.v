...
module c_tests_tb();
  ...
  localparam BAUD_PERIOD     = 1_000_000_000 / BAUD_RATE; // 1000 ns
...
SMU_RV32I_System # (
  ...
  initial begin
    ...
    forever begin
      ...
      #(BAUD_PERIOD + (BAUD_PERIOD / 2));  // sample at the center of bit 0

      for (uart_char = 0; uart_char < 8; uart_char = uart_char + 1) begin
        uart_byte[uart_char] = uart_txd;
        #BAUD_PERIOD;
      end

      $write("%c", uart_byte);
      #(BAUD_PERIOD / 2);                  // finish stop bit before next start
    end
  end
